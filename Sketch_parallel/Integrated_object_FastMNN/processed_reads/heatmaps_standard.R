#' @title Generate Cluster Composition Heatmaps from Seurat Object
#' @description This script reads a pre-clustered Seurat object, filters out
#'              "unknown" cell types, and generates heatmaps visualizing the
#'              composition of each cluster across various resolutions.
#'              Each heatmap displays the proportion of different cell types
#'              within each cluster.
#' @details The script sets a working directory, loads a Seurat object, and then
#'          iterates through a predefined set of clustering resolutions. For each
#'          resolution, it calculates cell type proportions per cluster and
#'          visualizes these proportions as a heatmap. The output is saved as
#'          a PDF file. Cells identified as "unknown", "Unknown", or `NA` in
#'          the specified annotation column are excluded from the analysis.
#' @param seurat_obj A Seurat object containing clustering results (e.g., in
#'                   `@meta.data` columns like `integrated_snn_res.X`) and
#'                   cell type annotations.
#' @param annot A character string specifying the name of the metadata column
#'              in `seurat_obj` that contains cell type annotations.
#' @param resolutions A character vector of numeric strings (e.g., "0.1", "0.5")
#'                    representing the different clustering resolutions to analyze.
#' @return A PDF file named "Cluster_Composition_Final_Heatmaps_<annot>.pdf"
#'         is generated in the working directory, containing one heatmap for
#'         each specified clustering resolution.
#' @author [Your Name/Organization]
#' @seealso \code{\link[Seurat]{Seurat}}, \code{\link[ggplot2]{ggplot}}, \code{\link[reshape2]{dcast}}
#' @import Seurat
#' @import ggplot2
#' @import reshape2
library(Seurat)
